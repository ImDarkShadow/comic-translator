...
const print = async (cluster,ocrData) => {
  try {
    console.log('top print -------');
    //let image = await Jimp.read('./output/004.jpg');
    for(imageNumber = 1; imageNumber <= ocrData.length  ; imageNumber++) {
      if (typeof ocrData[imageNumber - 1].ParsedResults[0].TextOverlay !== 'undefined') {
        console.log(imageNumber);
        let image = await Jimp.read(`./images/${imageNumber}.jpg`);
